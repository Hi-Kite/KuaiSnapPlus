	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		switchHideIcon.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				...
				Toast.makeText(SettingsActivity.this, isChecked ? "桌面图标已隐藏，可通过模块管理器访问" : "桌面图标已恢复显示", Toast.LENGTH_LONG)
						.show();
			}
		});
		...
		switchUseRoot.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			public void onCheckedChanged(CompoundButton buttonView, final boolean isChecked) {
				if (isChecked) {
							...
							new PermissionHelper.PermissionCallback() {
								public void onPermissionResult(boolean granted, String type) {
									if (granted) {
										...
										Toast.makeText(SettingsActivity.this, "已启用Root权限模式", Toast.LENGTH_SHORT).show();
									} else {
					...
					String[] pkgs = {"com.kuaiduizuoye.scan"};

					File publicDir = new File(Environment.getExternalStorageDirectory(), ".Kuaisnap");
					File publicFile = new File(publicDir, "toast_config");

					for (String pkg : pkgs) {
						String privateFile = "/storage/emulated/0/Android/data/" + pkg
								+ "/files/.Kuaisnap/toast_config";
						try {
							Runtime.getRuntime()
									.exec(new String[]{cmd, "-c",
											"if [ -f '" + privateFile + "' ]; then " + "  mkdir -p '"
													+ publicDir.getAbsolutePath() + "' && " + "  mv '" + privateFile
													+ "' '" + publicFile.getAbsolutePath() + "' ; " + "fi"})
									.waitFor();
		...
		switchRevoke.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					...
					new AlertDialog.Builder(SettingsActivity.this).setTitle("确认撤回")
							.setMessage("确定要撤回软件使用声明吗？软件将会重载，您需再次同意声明才能使用。").setCancelable(false)
							.setPositiveButton("确认", new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog, int which) {
									SharedPreferences.Editor editor = getSharedPreferences(MainActivity.SP_NAME,
											MODE_PRIVATE).edit();
	...
	private void ensureToastConfigInCorrectPlace(boolean suppressToast) {
		...
		if (suppressToast) {
			if (hasRootMode) {
				...
				String cmd = su.length() == 0 ? "su" : su;
				for (String pkg : pkgs) {
					String dir = "/storage/emulated/0/Android/data/" + pkg + "/files/.Kuaisnap";
			...
			if (hasRootMode) {
				...
				String cmd = su.length() == 0 ? "su" : su;
				for (String pkg : pkgs) {
					String file = "/storage/emulated/0/Android/data/" + pkg + "/files/.Kuaisnap/toast_config";
					try {
